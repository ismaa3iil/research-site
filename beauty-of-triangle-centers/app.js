(() => {
  ...
  function prefetchRank(rank) {
    ...
    [pair.interestingness, pair.quadtree].forEach(diagram => {
      const image = new Image();
      image.src = diagram.displayImage || diagram.image;
    });
  }
  ...
  function openImage(diagram) {
    ...
    imageDialogCaption.textContent = diagram.displayImage
      ? `${diagram.definition} · lossless ${format.integer(diagram.displayWidth)} × ${format.integer(diagram.displayHeight)} crop from the 6,667 × 5,779 source PNG`
      : `${diagram.definition} · 6,667 × 5,779 PNG`;
  ...
  async function initialize() {
    try {
      const response = await fetch("data/atlas.json?v=20260925-tight-crops");
